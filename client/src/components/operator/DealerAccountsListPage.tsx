...
const STATUS_COLOR: Record<string, string> = {
  active: "#16a34a", pending: "#f48120", suspended: "#c0392b",
};
          ...
          <tbody>
            {dealerships.map(d => (
              <tr key={d.id} onClick={() => navigate(`/operator-v6/dealerships/${d.id}`)}
                ...
                <td><span style={{padding: "2px 8px", borderRadius: 10, background: STATUS_COLOR[d.status] || "#888", color: "white", fontSize: 10, fontWeight: 600}}>{d.status || "unknown"}</span></td>
                <td>{d.unitCount}</td>
                <td>{d.claimCount}</td>
