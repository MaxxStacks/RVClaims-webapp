...
<nav className="sidebar">
  <div className="sidebar-logo"><svg width="36" height="36" viewBox="0 0 36 36"><rect width="36" height="36" rx="8" fill="var(--brand)"/><path d="M9 25L18 11L27 25H9Z" fill="white" opacity="0.8"/></svg><div className="sidebar-logo-text"><div className="sidebar-logo-name">Smith's RV Centre</div><div className="sidebar-logo-sub">Customer Portal</div></div><span className="sidebar-badge">Customer</span></div>
  ...
  <div className="sidebar-footer"><div className="user-info" onClick={() => showPage('settings')}><div className="user-avatar" id="cust-avatar">RM</div><div><div className="user-name">Robert Martin</div><div className="user-role">Customer</div></div></div></div>
</nav>
<div className="main">
...
<div className={`page ${activePage === 'settings' ? 'active' : ''}`} id="page-settings">
  <div style={{display: 'grid', gridTemplateColumns: '200px 1fr', gap: 24}}>
    ...
    <div>
      <div className={`pn cstab ${custSettingsTab === "cstab-cs-profile" ? "active" : ""}`} id="cstab-cs-profile" style={{display: custSettingsTab === "cstab-cs-profile" ? "block" : "none"}}><div className="pn-h"><span className="pn-t">My Profile</span></div>
        <div style={{padding: '24px 20px', display: 'flex', gap: 24, alignItems: 'flex-start', borderBottom: '1px solid #f0f0f0'}}>
          ...
          <div><div style={{fontSize: 16, fontWeight: 600, marginBottom: 2}}>Robert Martin</div><div style={{fontSize: 13, color: '#888'}}>Customer · Smith's RV Centre</div></div>
        </div>
    ...
    <MobileBottomNav portalType="customer" activePage={activePage} onNavigate={showPage} parents={parents} />
    <OfflineBanner />
    </>
  );
}
