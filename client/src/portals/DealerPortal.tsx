...
export default function DealerPortal() {
  ...
  const titles: Record<string, [string, string]> = {dashboard:['Dashboard',''],upload:['Upload Photos','Push to Claim'],claims:['My Claims',''],'claim-detail':['Claim Detail',''],units:['My Units',''],'add-unit':['Add New Unit','Register unit'],'unit-detail':['Unit Detail',''],
'svc-financing':['Financing','My financing requests'],'svc-financing-det':['Financing Request','Financing detail'],'svc-financing-new':['Request Financing','Submit to lenders'],
...
'svc-warranty':['Warranty Plans','Warranty plans'],'svc-warranty-detail':['Warranty Plan','Plan details \u0026 coverage'],'svc-parts':['Parts Orders','My parts orders'],'svc-parts-new':['Order Parts','Request parts'],'svc-parts-det':['Parts Order','Order status \u0026 tracking'],
'mkt-gate':['Dealer Marketplace','Membership required'],'mkt-browse':['Browse Marketplace','Units available'],'mkt-listing-view':['Unit Details','Marketplace listing'],'mkt-post-unit':['Post a Unit','List on marketplace'],'mkt-my-listings':['My Listings','Units you have listed'],'mkt-my-transactions':['My Transactions','Purchases & sales'],'mkt-live-auctions':['Live Auctions','Bid on units'],'mkt-auction-room':['Auction Room','Live bidding'],'mkt-showcase':['Public Showcase','$299/year add-on'],'mkt-showcase-submit':['Submit Unit','Public auction showcase'],
invoices:['Invoices \u0026 Billing','Payment history'],customers:['Customer Portal','Manage customer access'],'invite-customer':['Invite Customer','Send portal invitation'],'cust-tickets':['Customer Tickets','Support tickets from customers'],'cust-ticket-detail':['Customer Ticket','Ticket detail'],
...
clients:['Client Files','Your customer accounts'],messages:['Messages','Inbox from DS360 \u0026 claim threads'],documents:['Documents','Invoices, contracts \u0026 reports'],techflow:['TechFlow','Work orders \u0026 dispatch'],'wo-new':['New Work Order','Create a work order'],'wo-detail':['Work Order','Work order detail'],consignment:['Consignment','Manage consigned units'],marketing:['Marketing','Campaigns \u0026 lead capture'],'sales-services':['Sales \u0026 Services','Sell products, warranty plans \u0026 F\u0026I'],
...
<nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
  <div className="sidebar-logo" id="sidebar-header"><img src={ds360Icon} id="sidebar-logo-svg" width="36" height="36" style={{borderRadius:8}} alt="DS360" /><div className="sidebar-logo-text"><div className="sidebar-logo-name" id="sidebar-name">{user?.name ?? 'My Dealership'}</div><div className="sidebar-logo-sub">Dealership Portal</div><span className="sidebar-badge" style={{marginTop:4}}>{roleBadge}</span></div></div>
  <div className="sidebar-nav">
    ...
    {(showFinancing || showFI || showWarranty || showParts) && <div className="nav-section"><div className="nav-label">Services</div>
      {showFinancing && <div className={`nav-item ${isNavActive('svc-financing') ? 'active' : ''}`} onClick={() => showPage('svc-financing')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>Financing</div>}
      ...
      {showWarranty && <div className={`nav-item ${isNavActive('svc-warranty') ? 'active' : ''}`} onClick={() => showPage('svc-warranty')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 12l2 2 4-4"/></svg>Warranty Plans</div>}
      {showParts && <div className={`nav-item ${isNavActive('svc-parts') ? 'active' : ''}`} onClick={() => showPage('svc-parts')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>Parts Orders</div>}
    </div>}
    {!isTechnician && !isServiceRole && <div className="nav-section"><div className="nav-label">Marketplace</div>
      {showBrowse && <div className={`nav-item ${isNavActive('mkt-browse') ? 'active' : ''}`} onClick={() => showPage('mkt-browse')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>Browse Units</div>}
      ...
      {showAuctions && <div className={`nav-item ${isNavActive('mkt-live-auctions') ? 'active' : ''}`} onClick={() => showPage('mkt-live-auctions')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/></svg>Live Auctions</div>}
    ...
    {(showClients || showMessages || showDocuments || showTechflow || showConsignment || showMarketing || showSalesServices) && <div className="nav-section"><div className="nav-label">Operations</div>
      ...
      {showMessages && <div className={`nav-item ${isNavActive('messages') ? 'active' : ''}`} onClick={() => showPage('messages')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>Messages</div>}
    ...
    {isConsignor && <div className="nav-section"><div className="nav-label">My Consignment</div>
      ...
      <div className={`nav-item ${isNavActive('consignor-offers') ? 'active' : ''}`} onClick={() => showPage('consignor-offers')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>Offers Received</div>
    ...
    <div className="nav-section"><div className="nav-label">Operations</div>
      ...
      <div className={`nav-item ${isNavActive('svc-parts') ? 'active' : ''}`} onClick={() => showPage('svc-parts')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>Parts Orders</div>
      <div className={`nav-item ${isNavActive('messages') ? 'active' : ''}`} onClick={() => showPage('messages')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>Messages</div>
    </div></>}
    ...
    {showCustomers && <div className="nav-section"><div className="nav-label">Customers</div>
      <div className={`nav-item ${isNavActive('customers') ? 'active' : ''}`} onClick={() => showPage('customers')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>Customer Portal</div>
      <div className={`nav-item ${isNavActive('cust-tickets') ? 'active' : ''}`} onClick={() => showPage('cust-tickets')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>Customer Tickets</div></div>}
    <div className="nav-section"><div className="nav-label">Settings</div>
      ...
      <div className={`nav-item ${isNavActive('notifications') ? 'active' : ''}`} onClick={() => showPage('notifications')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>{(() => { const unread = dlrNotifications.filter((n: any) => !n.isRead).length; return <>{'Notifications'}{unread > 0 && <span className="nb red">{unread}</span>}</>; })()}</div>
