...
export default function OperatorPortal() {
  ...
  // Fallback FRC codes used when the API returns no codes for this manufacturer
  ...
  const titles: Record<string, [string, string]> = {dashboard:['Dashboard','Overview'],queue:['Processing Queue','Photo batches in queue'],'batch-review':['Photo Review','Batch review'],claims:['All Claims','All claims'],'claim-detail':['Claim Detail','Claim detail'],stale:['Stale Claims','Claims 36+ hours'],dealers:['Dealer Management','All dealers'],
'add-dealer':['Add New Dealer','Create dealership'],'dealer-detail':['Dealer Detail','Dealer account'],units:['Unit Inventory','All units'],'add-unit':['Add New Unit','Register unit'],'unit-detail':['Unit Detail','Unit specs'],frc:['FRC Codes','Manufacturer codes'],
marketplace:['Service Marketplace','Manage services'],'svc-financing':['Financing Services','Financing requests'],'svc-financing-detail':['Financing Detail','Financing request'],'svc-financing-new':['New Financing Request','Submit for dealer'],
'svc-fi':['F\u0026I Services','F\u0026I deals'],'svc-fi-detail':['F&I Detail','F&I deal'],'svc-fi-new':['New F\u0026I Deal','Flag for products'],
'svc-warranty':['Warranty Plans','Warranty plans'],'svc-warranty-new':['Add Warranty Plan','Register or sell'],
'svc-parts':['Parts \u0026 Accessories','Parts orders'],'svc-parts-detail':['Parts Order','Parts order'],'svc-parts-new':['New Parts Order','Request parts'],
'mkt-members':['Marketplace Members','Member dealers'],'mkt-member-detail':['Member Detail','Dealer verification'],'mkt-member-signup':['New Application','Review & approve'],'mkt-listings':['All Listings','142 active'],'mkt-listing-detail':['Listing Detail','Unit details'],'mkt-transactions':['Transactions','Escrow & commissions'],'mkt-transaction-detail':['Transaction Detail','Escrow tracking'],'mkt-auctions':['Live Auctions','Manage auctions'],'mkt-auction-detail':['Auction Detail','Bidding & settlement'],'mkt-public-events':['Public Auction Events','Monthly public sales'],'mkt-public-event-detail':['Event Detail','Manage showcase'],
...
'ops-dashboard':['Operations Dashboard','Active claims, work orders & live queue'],'mfr-portals':['Manufacturer Portals','Quick-launch manufacturer claim systems'],'financing-apps':['Financing Applications','Review & submit to lenders'],'techflow-oversight':['TechFlow Oversight','Work orders across all dealers'],'financing-partners':['Financing Partners','Lenders, rates & partner management'],'parts-catalog':['Parts Catalog','Global parts & accessories catalog'],'communications':['Communications Hub','Broadcast messages & push notifications']};
  ...
  const selectedBatch = opBatches.find((b: any) => b.id === selectedBatchId) ?? null;
  const selectedUnit = opUnits.find((u: any) => u.id === selectedUnitId) ?? null;
...
<nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
  ...
  <div className="sidebar-nav">
    ...
    <div className="nav-section"><div className="nav-label">Claims</div>
      <div className={`nav-item ${isNavActive('queue') ? 'active' : ''}`} onClick={() => showPage('queue')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>Processing Queue{opBatches.length > 0 && <span className="nb red">{opBatches.length}</span>}</div>
      <div className={`nav-item ${isNavActive('claims') ? 'active' : ''}`} onClick={() => showPage('claims')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>All Claims{opClaims.length > 0 && <span className="nb blue">{opClaims.length}</span>}</div>
      <div className={`nav-item ${isNavActive('stale') ? 'active' : ''}`} onClick={() => showPage('stale')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>Stale Claims{staleClaimCount > 0 && <span className="nb amber">{staleClaimCount}</span>}</div></div>
    <div className="nav-section"><div className="nav-label">Management</div>
      <div className={`nav-item ${isNavActive('dealers') ? 'active' : ''}`} onClick={() => showPage('dealers')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>Dealers{pendingDealerCount > 0 && <span className="nb amber">{pendingDealerCount}</span>}</div>
    ...
    <div className="nav-section"><div className="nav-label">Services</div>
      ...
      <div className={`nav-item ${isNavActive('svc-financing') ? 'active' : ''}`} onClick={() => showPage('svc-financing')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>Financing</div>
      <div className={`nav-item ${isNavActive('financing-apps') ? 'active' : ''}`} onClick={() => showPage('financing-apps')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>Financing Apps</div>
      ...
      <div className={`nav-item ${isNavActive('svc-fi') ? 'active' : ''}`} onClick={() => showPage('svc-fi')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>F&I Services</div>
      <div className={`nav-item ${isNavActive('svc-warranty') ? 'active' : ''}`} onClick={() => showPage('svc-warranty')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 12l2 2 4-4"/></svg>Warranty Plans</div>
      <div className={`nav-item ${isNavActive('svc-parts') ? 'active' : ''}`} onClick={() => showPage('svc-parts')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 7h-9"/><path d="M14 17H5"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/></svg>Parts & Accessories</div>
    ...
    <div className="nav-section"><div className="nav-label">Marketplace</div>
      <div className={`nav-item ${isNavActive('mkt-members') ? 'active' : ''}`} onClick={() => showPage('mkt-members')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>Members</div>
      <div className={`nav-item ${isNavActive('mkt-listings') ? 'active' : ''}`} onClick={() => showPage('mkt-listings')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4a2 2 0 012 2v6a2 2 0 01-2 2h-4"/></svg>Listings</div>
      ...
      <div className={`nav-item ${isNavActive('mkt-auctions') ? 'active' : ''}`} onClick={() => showPage('mkt-auctions')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/></svg>Live Auctions</div>
    ...
    <div className="nav-section"><div className="nav-label">System</div>
      ...
      <div className={`nav-item ${isNavActive('blog') ? 'active' : ''}`} onClick={() => showPage('blog')}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>Blog</div>
...
<div className={`page ${activePage === 'dashboard' ? 'active' : ''}`} id="page-dashboard">
  <div className="al-g">
    ...
    <div className="al"><div className="al-i wa"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div><div className="al-c"><div className="al-t">{staleClaimCount} stale claim{staleClaimCount !== 1 ? "s" : ""}</div><div className="al-d">No update in 36+ hours</div></div><span className="al-a" onClick={() => showPage('stale')}>View</span></div>
    <div className="al"><div className="al-i in"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div><div className="al-c"><div className="al-t">{opBatches.length} photo batch{opBatches.length !== 1 ? "es" : ""} in queue</div><div className="al-d">Awaiting review & FRC sorting</div></div><span className="al-a" onClick={() => showPage('queue')}>Process</span></div>
  </div>
...
<div className={`page ${activePage === 'batch-review' ? 'active' : ''}`} id="page-batch-review">
  <div className="detail-header"><button className="detail-back" onClick={() => showPage('queue')}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg></button><div className="detail-info"><div className="detail-title">{selectedBatch?.batchNumber ?? "—"} — Photo Review</div><div className="detail-meta">{selectedBatch?.dealerName ?? "—"} · {selectedBatch?.claimType ?? ""}{selectedBatch?.vin ? ` · VIN: ${selectedBatch.vin}` : ""} · {selectedBatch?.photoCount ?? 0} photos</div></div><button className="btn btn-s btn-sm" onClick={() => handleToast('Claim created from groups')}>Create Claim from Groups</button></div>
  <div style={{display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20}}>
    ...
    <div>
      ...
      <button style={{width: '100%', padding: 12, border: '2px dashed #d0d0d0', borderRadius: 8, background: 'none', color: '#888', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 16}} onClick={() => handleToast('Issue group added')}>+ Add Issue Group</button>
...
<div className={`page ${activePage === 'claim-detail' ? 'active' : ''}`} id="page-claim-detail">
  ...
  <div className="cd-grid">
    ...
    <div>
      ...
      <div className="cd-section"><div className="cd-section-h">Dealer</div><div className="cd-row"><span className="cd-label">Name</span><span className="cd-value cid" onClick={() => showPage("dealer-detail")}>{selectedClaimDetail?.dealerName ?? "—"}</span></div><div className="cd-row"><span className="cd-label">Plan</span><span className="cd-value">{selectedClaimDetail?.dealerPlan ?? "—"}</span></div></div>
...
<div className={`page ${activePage === 'dealers' ? 'active' : ''}`} id="page-dealers">
  <div className="tabs"><div className="tab active" onClick={(e) => switchTab(e)}>Active ({opDealers.filter((d:any)=>d.status==='active').length})</div><div className="tab" onClick={(e) => switchTab(e)}>Pending ({pendingDealerCount})</div><div className="tab" onClick={(e) => switchTab(e)}>Suspended ({opDealers.filter((d:any)=>d.status==='suspended').length})</div></div>
...
<div className={`page ${activePage === 'dealer-detail' ? 'active' : ''}`} id="page-dealer-detail">
  ...
  <div style={{display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 16, marginBottom: 20}}>
    <div className="sc"><div className="sc-l" style={{marginBottom: 8}}>Batches</div><div className="sc-v" style={{color: '#f59e0b'}}>{opBatches.filter((b:any)=>b.dealershipId===selectedDealerId).length}</div></div>
    <div className="sc"><div className="sc-l" style={{marginBottom: 8}}>Claims</div><div className="sc-v">{opClaims.filter((cl:any)=>cl.dealershipId===selectedDealerId).length}</div></div>
  ...
  <div className={`pn dtab ${dealerTab === "dtab-d-claims" ? "active" : ""}`} id="dtab-d-claims" style={{display: dealerTab === "dtab-d-claims" ? "block" : "none"}}><div className="tw"><table><thead><tr><th>Claim #</th><th>VIN</th><th>Type</th><th>Status</th><th>Amount</th><th>Updated</th></tr></thead><tbody><tr><td><span className="cid" onClick={() => showPage('claim-detail')}>CLM-0248</span></td><td><span className="vin">...4K1</span></td><td>Warranty</td><td><span className="bg submitted">Submitted</span></td><td>$1,240</td><td>2h ago</td></tr><tr><td><span className="cid" onClick={() => showPage('claim-detail')}>CLM-0243</span></td><td><span className="vin">...7P3</span></td><td>DAF</td><td><span className="bg pay-recv">Paid</span></td><td>$4,200</td><td>3 days</td></tr><tr><td><span className="cid" onClick={() => showPage('claim-detail')}>CLM-0237</span></td><td><span className="vin">...8R2</span></td><td>Warranty</td><td><span className="bg completed">Completed</span></td><td>$920</td><td>1 week</td></tr></tbody></table></div></div>
...
<div className={`page ${activePage === 'add-unit' ? 'active' : ''}`} id="page-add-unit">
  ...
  <div className="pn"><div className="form-grid c3">
    ...
    <div className="form-group"><label>Lot Location</label><input placeholder="Lot B, Row 3" /></div><div className="form-group"><label>Status</label><select><option>On Lot</option><option>Delivered</option><option>In Service</option></select></div><div className="form-group"><label>Dealer</label><select value={addUnitForm.dealershipId} onChange={e => setAddUnitForm(f => ({...f, dealershipId: e.target.value}))}><option value="">Select...</option>{opDealers.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
  ...
  </div><div className="btn-bar"><button className="btn btn-p" onClick={() => handleCreateUnit()} disabled={addUnitSaving}>{addUnitSaving ? 'Saving...' : 'Add Unit'}</button><button className="btn btn-o" onClick={() => showPage('units')}>Cancel</button></div></div>
</div>
...
<div className={`page ${activePage === 'unit-detail' ? 'active' : ''}`} id="page-unit-detail">
  ...
  <div className={`pn utab ${unitTab === "utab-u-claims" ? "active" : ""}`} id="utab-u-claims" style={{display: unitTab === "utab-u-claims" ? "block" : "none"}}><div className="tw"><table><thead><tr><th>Claim #</th><th>Type</th><th>Lines</th><th>Status</th><th>Claimed</th><th>Approved</th></tr></thead><tbody><tr><td><span className="cid" onClick={() => showPage('claim-detail')}>CLM-0248</span></td><td>Warranty</td><td>4</td><td><span className="bg submitted">Submitted</span></td><td>$1,240</td><td>—</td></tr><tr><td><span className="cid" onClick={() => showPage('claim-detail')}>CLM-0237</span></td><td>DAF</td><td>7</td><td><span className="bg pay-recv">Paid</span></td><td>$4,200</td><td>$3,920</td></tr><tr><td><span className="cid" onClick={() => showPage('claim-detail')}>CLM-0225</span></td><td>PDI</td><td>3</td><td><span className="bg pay-recv">Paid</span></td><td>$920</td><td>$840</td></tr></tbody></table></div></div>
...
<div className={`page ${activePage === 'svc-parts-new' ? 'active' : ''}`} id="page-svc-parts-new">
  ...
  <div className="pn"><div className="form-grid"><div className="form-group"><label>Dealer</label><select><option value=''>Select...</option>{opDealers.map((d:any)=><option key={d.id} value={d.id}>{d.name}</option>)}</select></div><div className="form-group"><label>Related Claim</label><select><option value=''>None</option>{opClaims.map((cl:any)=><option key={cl.id} value={cl.id}>{cl.claimNumber}</option>)}</select></div><div className="form-group full"><label>Items Needed</label><textarea placeholder="List parts needed, part numbers if known, quantities...">1x Sidewall panel (2x3 section)
