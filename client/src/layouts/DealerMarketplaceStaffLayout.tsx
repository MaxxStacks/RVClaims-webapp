...
export default function DealerMarketplaceStaffLayout({ children }: Props) {
  ...
  const base = `/${dealerId}/marketplace-staff`;
  const isActive = (path: string) => location.includes(`/marketplace-staff/${path}`);
      ...
      <nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
        ...
        <div className="sidebar-nav">
          <div className="nav-section">
            ...
            <Link className={`nav-item ${isActive('bids') ? 'active' : ''}`} to={`${base}/bids`}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>Bids</Link>
          </div>
        </div>
        <div className="sidebar-footer">
          <div className="user-info" style={{cursor:'pointer'}}>
            <div className="user-avatar">{user?.email ? user.email.slice(0,2).toUpperCase() : 'DS'}</div>
            <div>
        ...
        <header className="header">
          <div className="header-left">
            ...
            <div>
              ...
              <div className="header-sub">Dealer Staff (Read-Only)</div>
            </div>
          </div>
