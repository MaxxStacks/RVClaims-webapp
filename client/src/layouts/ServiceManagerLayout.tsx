...
export default function ServiceManagerLayout({ children }: Props) {
      ...
      <nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
        <div className="sidebar-logo">
          ...
          <div className="sidebar-logo-text">
            <div className="sidebar-logo-sub" style={{fontSize:12,fontWeight:600}}>Service Portal</div>
            <span className="sidebar-badge" style={{marginTop:4}}>Svc Mgr</span>
          </div>
        </div>
        <div className="sidebar-nav">
          ...
          <div className="nav-section">
            ...
            <Link className={`nav-item ${isActive('work-orders') ? 'active' : ''}`} to="work-orders"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg>Work Orders</Link>
            <Link className={`nav-item ${isActive('scheduling') ? 'active' : ''}`} to="scheduling"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>Scheduling</Link>
            <Link className={`nav-item ${isActive('technicians') ? 'active' : ''}`} to="technicians"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>Technicians</Link>
            ...
            <Link className={`nav-item ${isActive('service-appointments') ? 'active' : ''}`} to="service-appointments"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07"/></svg>Service Appointments</Link>
        ...
        <div className="sidebar-footer">
          <div className="user-info" style={{cursor:'pointer'}}>
            <div className="user-avatar">{user?.name ? user.name.split(' ').filter(Boolean).map((n: string) => n[0]).join('').toUpperCase().slice(0,2) : 'SM'}</div>
            <div>
      ...
      <div className={`main${sidebarCollapsed ? ' collapsed-main' : ''}`}>
        <header className="header">
          <div className="header-left">
            ...
            <div>
              <div className="header-title">Service Portal</div>
              <div className="header-sub">Service Manager</div>
            </div>
          </div>
