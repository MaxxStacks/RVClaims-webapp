...
export default function MarketplaceAdminLayout({ children }: Props) {
      ...
      <nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
        <div className="sidebar-logo">
          ...
          <div className="sidebar-logo-text">
            ...
            <span className="sidebar-badge" style={{marginTop:4}}>Admin</span>
          </div>
        </div>
        <div className="sidebar-nav">
          ...
          <div className="nav-section">
            ...
            <Link className={`nav-item ${isActive('listings') ? 'active' : ''}`} to="listings"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>All Listings</Link>
            <Link className={`nav-item ${isActive('auctions') ? 'active' : ''}`} to="auctions"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2l-2 2-7 7 3 3 7-7 2-2-3-3z"/><path d="M3 21l4-4"/><path d="M17 3l4 4-2 2-4-4 2-2z"/></svg>All Auctions</Link>
            <Link className={`nav-item ${isActive('bids') ? 'active' : ''}`} to="bids"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>Bids &amp; Offers</Link>
            <Link className={`nav-item ${isActive('escrow') ? 'active' : ''}`} to="escrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>Escrow Management</Link>
          </div>
          <div className="nav-section">
            <div className="nav-label">Members</div>
            <Link className={`nav-item ${isActive('members') ? 'active' : ''}`} to="members"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>Members</Link>
          </div>
          <div className="nav-section">
            <div className="nav-label">Finance</div>
            <Link className={`nav-item ${isActive('revenue') ? 'active' : ''}`} to="revenue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>Revenue</Link>
          </div>
          <div className="nav-section">
        ...
        <div className="sidebar-footer">
          <div className="user-info" style={{cursor:'pointer'}}>
            <div className="user-avatar">{user?.email ? user.email.slice(0,2).toUpperCase() : 'MA'}</div>
            <div>
              <div className="user-name">{user?.email ?? 'Operator'}</div>
              <div className="user-role">Marketplace Admin</div>
            </div>
          </div>
      ...
      <div className={`main${sidebarCollapsed ? ' collapsed-main' : ''}`}>
        <header className="header">
          <div className="header-left">
            ...
            <div>
              <div className="header-title">Marketplace Admin</div>
