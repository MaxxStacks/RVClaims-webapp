      ...
      <nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
        <div className="sidebar-logo">
          ...
          <div className="sidebar-logo-text">
            <div className="sidebar-logo-sub" style={{fontSize:12,fontWeight:600}}>Field Tech Portal</div>
            <span className="sidebar-badge" style={{marginTop:4}}>On-Site Tech</span>
          </div>
        </div>
        ...
        <div className="sidebar-footer">
          <div className="user-info" style={{cursor:'pointer'}}>
            <div className="user-avatar">{user?.name ? user.name.split(' ').filter(Boolean).map((n: string) => n[0]).join('').toUpperCase().slice(0,2) : 'OT'}</div>
            <div>
              <div className="user-name">{user?.name ?? user?.email ?? 'Technician'}</div>
              <div className="user-role">On-Site Technician</div>
            </div>
          </div>
      ...
      <div className={`main${sidebarCollapsed ? ' collapsed-main' : ''}`}>
        <header className="header">
          <div className="header-left">
            ...
            <div>
              <div className="header-title">Field Tech Portal</div>
              <div className="header-sub">On-Site Technician</div>
            </div>
          </div>
