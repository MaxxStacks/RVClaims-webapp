...
export default function DealerStaffLayout({ children }: Props) {
      ...
      <nav className={`sidebar${sidebarCollapsed ? ' collapsed' : ''}`}>
        ...
        <div className="sidebar-footer">
          <div className="user-info" style={{cursor:'pointer'}}>
            <div className="user-avatar">{user?.name ? user.name.split(' ').filter(Boolean).map((n: string) => n[0]).join('').toUpperCase().slice(0,2) : 'DS'}</div>
            <div>
              <div className="user-name">{user?.name ?? user?.email ?? 'Staff'}</div>
        ...
        <header className="header">
          <div className="header-left">
            ...
            <div>
              ...
              <div className="header-sub">Dealer Staff</div>
            </div>
          </div>
