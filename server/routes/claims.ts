// server/routes/claims.ts — Claims API (/api/claims)
...
// Also accessible via /api/frc-codes (registered separately in index.ts)
...
// GET /api/claims — scoped by role with filters
...
// GET /api/claims/:id — full unit context for operator
router.get("/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims GET /:id]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

// POST /api/claims — create draft claim (Approach A: draft first, then submit)
...
// POST /api/claims/:id/submit — transitions draft → new_unassigned (requires photos)
router.post("/:id/submit", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims POST /:id/submit]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

// DELETE /api/claims/:id — delete draft claim on cancel
router.delete("/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims DELETE /:id]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

// POST /api/claims/:id/assign — operator assigns claim to self or staff
router.post("/:id/assign", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims POST /:id/assign]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.post("/:id/request-info", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims POST /:id/request-info]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.post("/:id/approve", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims POST /:id/approve]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.post("/:id/deny", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims POST /:id/deny]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

// POST /api/claims/:id/transition — operator transitions status (full state machine)
router.post("/:id/transition", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (toStatus === "closed" || toStatus === "completed") {
      ...
      ;(async () => {
        try {
          ...
          console.error("[claims/transition] review trigger failed:", reviewErr);
        }
      })();
    }
    ...
    console.error("[claims POST /:id/transition]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

// PATCH /api/claims/:id — general status/notes update (alias for common field updates)
router.patch("/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("[claims PATCH /:id]", err);
    res.status(500).json({ error: "Internal server error" });
  }
});
