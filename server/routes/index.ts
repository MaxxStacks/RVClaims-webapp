...
import notificationDeliveriesRouter from './notification-deliveries';
import frcCodesRouter from './frc-codes';
import uploadsRouter from './uploads';
import partsOrdersRouter from './parts';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use('/api/frc-codes', frcCodesRouter);
  app.use('/api/uploads', uploadsRouter);
  app.use('/api/parts-orders', partsOrdersRouter);
  app.use('/api/notification-deliveries', notificationDeliveriesRouter);
  // Backward compatibility — redirect old v6 API paths to new paths
  app.use('/api/v6', (req, res) => {
    const newPath = req.originalUrl.replace('/api/v6', '/api');
